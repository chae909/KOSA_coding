	...
	public void showMenu() { // show method 만들기
		...
		do {
			...
			if (menu.equals("1")) { // 고객등록
				...
				id = readFromKeyboard();
				Customer cust = myBank.getCustomer(id); // 꼭 있는 아이디를 넣어서 테스트하자
				...
				if (cust == null) {
					...
					System.out.println("정상적으로 출금되었습니다.");
					System.out.println("예금후 잔고는 :" + cust.getAccount().getBalance());
				}
			}
		} while (!menu.equals("q"));
	}

	public String readFromKeyboard() { // nextLine과 같다
		String input = null;
		try {
